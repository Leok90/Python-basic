...
def memLog():
	...
	login_mem.append(ID)
	login_mem.append(PWD)
	for i in range(len(memList)):
		if ID in memList[i][0]:
			cntID += 1
	for i in range(len(memList)):
		if PWD in memList[i][1]:
			cntPWD += 1
	if cntID == 1 and cntPWD == 1:
		print("\n\t\t로그인에 성공하였습니다.")
		print("\t\t", login_mem)
	elif cntID == 0 and cntPWD == 1:
		print("\n\t\t ID를 확인해주세요.")
		print("\t\t", login_mem)
	elif cntID == 1 and cntPWD == 0:
		print("\n\t\tPassword를 확인해주세요.")
		print("\t\t", login_mem)
	elif cntID == 0 and cntPWD == 0:
